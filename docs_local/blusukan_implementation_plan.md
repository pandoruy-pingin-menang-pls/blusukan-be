...
| 2 | `feat/auth-otp-jwt` | `feat/core-infrastructure` | *(Maintainer)* |
| 3 | `feat/merchant-onboarding-ingestion` | `feat/auth-otp-jwt` | |
| 4 | `feat/events-admin-hitl` | `feat/auth-otp-jwt` | |
| 5 | `feat/inventory-predictive-stock` | `feat/merchant-onboarding-ingestion` + `feat/events-admin-hitl` | |
| 6 | `feat/routing-itinerary-generation` | `feat/auth-otp-jwt` + `feat/merchant-onboarding-ingestion` | |
...
- `app/db/migrations/versions/001_create_users_refresh_tokens.py`
...
| phone_number | VARCHAR(20) UNIQUE NOT NULL | Format E.164 (`+6281xxxxxxxx`) |
...
**Flow OTP Request:**
1. Validasi format E.164 di Pydantic schema (regex). Jika tidak valid → `400`.
2. Cek Redis counter `otp_rate:{phone_number}` → jika ≥ 3 dalam 10 menit → `429 OTP_RATE_LIMIT` + header `Retry-After`.
3. Generate OTP 6 digit → bcrypt hash → simpan di Redis key `otp:{phone_number}` TTL 300 detik.
4. Kirim OTP via provider. Jika provider error → propagate `502`.

**Flow OTP Verify:**
1. Ambil hash dari Redis key `otp:{phone_number}`. Jika tidak ada → `400 OTP_EXPIRED`.
2. Verifikasi bcrypt hash. Jika tidak cocok → `400 INVALID_OTP`.
3. **Hapus Redis key secara atomik** (DEL) → OTP single-use.
4. Upsert user (buat baru jika belum ada, fetch jika sudah ada).
...
|--------|-------|------|----------|------|
| POST | `/api/auth/otp/request` | `{phone_number}` | `202 {message, retry_after: 60}` | ❌ |
| POST | `/api/auth/otp/verify` | `{phone_number, otp_code}` | `200 {access_token, refresh_token, user}` | ❌ |
| POST | `/api/auth/refresh` | `{refresh_token}` | `200 {access_token, refresh_token}` | ❌ |
| GET | `/api/auth/me` | — | `200 {id, full_name, phone_number, has_merchant_profile, role}` | ✅ Bearer |
| PATCH | `/api/auth/me` | `{full_name?}` | `200 {user}` | ✅ Bearer |
| POST | `/api/auth/logout` | `{refresh_token}` | `204` | ✅ Bearer |

### Edge Cases Wajib Ditangani
- OTP spam → `429` + `Retry-After` header
- OTP expired (TTL habis) → `400 OTP_EXPIRED` bukan generic error
- Dua request verify OTP bersamaan dengan kode benar → Redis atomic DEL cegah double-use
- Refresh token revoked tapi dipakai lagi → invalidasi semua sesi user
- Akses endpoint privat tanpa token → `401` bukan `403`

### Checklist Merge ke `develop`
- [ ] OTP request → simpan di Redis, kirim ke provider
- [ ] OTP verify → JWT + refresh token direturn
- [ ] Single-use OTP terbukti (verifikasi ke-2 → `400`)
- [ ] Rate limit 3x/10 menit terbukti
...
- `app/db/migrations/versions/002_create_merchants_catalog.py`
...
- `app/db/migrations/versions/003_create_events.py`
...
- `app/db/migrations/versions/004_create_inventory_recommendations.py`
- `tests/unit/test_inventory_calculator.py` ← coverage ≥ 90%

### Deskripsi Fitur
> Setiap hari jam 05.00 WIB, Celery job menghitung rekomendasi stok untuk semua merchant aktif menggunakan formula deterministik (bukan LLM). Gemini hanya mengubah angka final menjadi kalimat saran (NLG layer saja). Merchant melihat rekomendasi di dashboard pagi.
...
1. Trigger: Celery beat setiap hari 05.00 WIB.
...
- `app/db/migrations/versions/005_create_itineraries.py`
...
3. Apply default fallback jika field kosong: `time_limit=240`, `radius=2000`, `budget=100000`.
4. Clamp bounds: `time_limit` ∈ [30, 480], `radius` ∈ [500, 5000], `budget` ≥ 10000.
5. PostGIS query kandidat merchant dalam radius.
6. Embed `interest_categories` → pgvector cosine similarity per merchant.
7. Merchant tanpa embedding → `category_match = 0.0` (jangan null/crash).
8. Hitung SAW score per kandidat (Python, bukan SQL).
9. Filter berdasarkan budget & estimasi waktu tempuh.
10. Sort Score DESC, top-N waypoint. Tie-break: `distance ASC`.
11. OSRM route → `route_geojson`, `estimated_duration_minutes`.
12. Jika `estimated_duration > time_limit` → drop waypoint terakhir, re-request OSRM (iterative trimming).
13. Simpan ke `itineraries`, return ke FE.
...
- `app/db/migrations/versions/006_create_transactions_stamps_promos.py`
...
**Tabel `promos`** + **`promo_redemptions`** (lihat schema lengkap di dokumen asli).

### Business Logic

**POS Log Transaksi:**
1. Merchant call `POST /api/merchants/{id}/transactions`.
2. Validasi `nominal_value > 0`.
3. Flag `is_suspicious = true` jika `nominal_value > 5.000.000`.
4. Cek `client_reference_id` idempotency jika disertakan.
5. Insert transaksi.
6. Jika `linked_itinerary_id` ada: validasi itinerary milik tourist yang login → trigger `gamification_service.award_stamp()` secara **async background**.

**Gamification Award Stamp (idempotent):**
1. Cek `UNIQUE(transaction_id)` di `stamps` → jangan double-award.
2. Insert stamp.
3. Hitung total stamp user → cek apakah memenuhi promo yang aktif.

**Promo Redemption:**
1. `POST /api/promos/{id}/redeem`.
2. **Row lock** (`SELECT ... FOR UPDATE`) saat hitung stamp vs `stamp_required_count` → cegah race condition.
3. Validate promo `is_active` + belum `valid_until` terlewati.
4. Generate `redemption_code` (8-char alphanumeric unik, uppercase).
5. Merchant confirm via `POST /api/merchants/{id}/promo-redemptions/{code}/confirm`.
...
- `app/db/migrations/versions/007_create_credit_score_logs.py`
