...
| Auth & Storage | Supabase Auth (Email + Password) + Supabase Storage (foto menu, foto produk) |
...
1. **Trigger:** Celery beat job harian (jam 03:00 WIB) `tasks_stock_recalc.py` untuk semua merchant `status='active'`. Juga bisa dipanggil manual via endpoint (misal setelah event baru di-approve).
...
2. **Parsing constraint:** panggil `gemini_client.parse_constraints(raw_query)` dengan strict JSON schema output:
   ...
   ```
   - Sertakan default value fallback: `time_limit_minutes = 300`, `search_radius_meter = 2000`, `budget_idr = 999999999`.
3. **Hitung Jumlah Toko & Alokasi Budget:**
   - Gunakan heuristik waktu: <60m=1, 60-120m=2, 120-180m=3, >180m=4 toko.
   - `budget_per_toko = budget_idr / jumlah_toko`.
4. **Candidate generation (PostGIS):**
   ```sql
   SELECT m.*, ST_Distance(m.location, ST_MakePoint(:lng,:lat)::geography) AS distance_m,
   ...
   GROUP BY m.id
   ```
5. **Scoring per kandidat (SAW formula, dihitung di Python, BUKAN di SQL agar mudah diuji unit):**
   ```
   HiddenGemIndex = 1 - min(review_count / MAX_REVIEW_THRESHOLD, 1)   # MAX_REVIEW_THRESHOLD misal 100
   CategoryMatch = max_category_match # Dari query SQL di atas
   DistanceNorm = 1 - (distance_m / max_radius_meter)   # clamp ke [0,1]
...
6. Sort by `Score` DESC, ambil potong sejumlah `jumlah_toko` yang dihitung dari poin 3.
7. Kirim ordered waypoints ke `osrm_client.get_route(waypoints)` 1x saja → dapatkan `route_geojson`, `total_distance`, `estimated_duration`.
8. Simpan ke `itineraries`, return ke FE.
9. `PATCH /itineraries/{id}/start` → set `status='active'`, mulai geofence tracking sisi FE (backend hanya expose endpoint check-in).
